...
The version-1 declaration-key scheme is `nominal_name_hash_v1`, wire tag `0`. Canonical names use the lowercase ASCII dotted grammar defined for feature and policy names. `FunctionKey` and `GroupKey` are 16-byte values. They are respectively `BLAKE3("weavy.function-key.v1\0" || utf8(canonical_name))[0..16]` and `BLAKE3("weavy.group-key.v1\0" || utf8(canonical_name))[0..16]`. `MemberKey` and `BlockKey` are `u32` values formed by interpreting the first four digest bytes as little-endian: `MemberKey = LE_u32(BLAKE3("weavy.member-key.v1\0" || GroupKey || utf8(canonical_local_name))[0..4])`; `BlockKey = LE_u32(BLAKE3("weavy.block-key.v1\0" || FunctionKey || utf8(canonical_local_name))[0..4])`. Fixed-width owner keys make the local projections unambiguous without a separator. Global keys sort by unsigned lexicographic raw bytes; local keys sort numerically. Declarations retain their canonical names and complete descriptors. Duplicate keys reject; a shared key with unequal retained names or unequal descriptors is a collision and rejects rather than aliasing. `TypeDigest` is the complete 32-byte BLAKE3 digest assigned by the canonical recursive-type descriptor projection; it is never truncated to a nominal key.

Every semantic operation has exactly one canonical `InstId = { function: FunctionKey, ordinal: u32 }`. The ordinal is zero-based canonical instruction order after producer canonicalization, independent of byte offset and dense aliases. `InstId`s sort by `FunctionKey` raw bytes and then numeric ordinal. Admission rejects duplicate, missing, noncanonical, or out-of-range identities.
...
Acyclic anonymous structural types are structurally interned. Every recursive declared strongly connected component uses `TypeKeyScheme::NominalNameHashV1`, the 16-byte `GroupKey` and member-local `u32` `MemberKey` projection defined above, and a 32-byte cryptographic `TypeDigest` over its canonical semantic descriptor. Keys are stable nominal names, not unsupported claims of global collision freedom. Admission canonicalizes member order by numeric `MemberKey`, recomputes the digest, rejects duplicate keys and unequal names or descriptors, and compares scheme, key, and digest at imports. Anonymous mutually recursive structural types are inadmissible in version 1.
